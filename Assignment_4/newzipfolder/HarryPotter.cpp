...
HarryPotter::HarryPotter(std::string n1) : Character()
{
	armor = 0;
	strength = 10;
	name = n1;
...
void HarryPotter::defense(int usrAttack, bool isM)
{
	...
	if (isM == true && usrAttack == 12)
	{
		...
		if (times_killed == 0)
		{
			...
			std::cout << "But wait, "<< name <<" has come back to life!" << std::endl;
			strength = 20;
			 times_killed = 1;
		}
		...
		if (strength < 0 && times_killed == 0) {
			std::cout << name <<" is dead, but he has come back to life!" << std::endl;
			strength = 20;
		}
		else if (strength < 0 && times_killed != 0)
			std::cout << name <<" has died (for real this time)!" << std::endl;
	}
}
