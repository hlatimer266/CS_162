...
int BlueMen::attack()
{
	...
	std::cout << "Blue Men attack pts = " << die1 + die2 << std::endl;
	return die1 + die2;
}
...
void BlueMen::defense(int usrAttack, bool isM)
{
	...
	if (isM == true && usrAttack == 12)
	{
		...
		std::cout << "Blue Men are dead!" << std::endl;
		strength = 0;
		...
		int sumDamge = 0;
		std::cout << "Blue Men armor = " << armor << std::endl;
		...
		else if ((usrAttack) >= 4)
		{
			...
			std::cout << "Blue men lose 1 defense die!" << std::endl;
			dieArray[numDie - 1] = { 0 };
			numDie--;
			count -= 4;
			...
			while (count > 0)
			{
				...
				{
					std::cout << "Blue men lose another defense die!" << std::endl;
					dieArray[numDie - 1] = { 0 };
					numDie--;
					count -= 4;
		...
		std::cout << "Blue Men defense pts = " << sumDamge << std::endl;
		damage = usrAttack - sumDamge - armor;

		if (damage <= 0)
			damage = 0;

		std::cout << "Damge taken by Blue Men = " << damage << std::endl;
		strength -= damage;
		std::cout << "strength left in the Blue Men = " << strength << std::endl;
